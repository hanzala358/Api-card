var data = {
  ...
  let displayContent = document.getElementById("main");
  
  let drop = document.getElementById("selectId");
  console.log(drop.value);
  
  var uniqueCategory = []
  
  for (let i = 0; i < uniqueCategory.length; i++) {
    drop.innerHTML += `<option value="${uniqueCategory[i].category}">${uniqueCategory[i].category}</option>`
  ...
  for (let i = 0; i < data.products.length; i++) {
    if (!uniqueCategory.includes(data.products[i].category)) {
      ...
      drop.innerHTML += `<option value="${optionValue}">${optionValue}</option>`
  ...
  for (let i = 0; i < data.products.length; i++) {
    ...
    displayContent.innerHTML += `<div class="card"><img class="one" src="${data.products[i].images}" alt="image">
    <h6>${data.products[i].category}</h6>
    <h6>${data.products[i].title}</h6>
  ...
  drop.addEventListener('change', () => {
    ...
    var selectedOpt = drop.value
    for (let i = 0; i < data.products.length; i++) {
      if (selectedOpt == data.products[i].category) {
        displayContent.innerHTML += `
         <div class="card" data-aos="fade-up">
         <img width="200px" height="200px" src="${data.products[i].images}" alt="image">
  ...
  for (let i = 2; i < data.products.length; i++) {
    var cardbtn = document.getElementById(`${i}`)
